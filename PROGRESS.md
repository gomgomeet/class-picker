...
## 2026-07-10 (금) — 에이전트 자동 동기화

### 오늘 한 일
- 에이전트가 추가될 때마다 개인 폴더(~/.claude)에 자동 설치되도록 훅 2개 추가
  - 세션 시작 시: git pull로 받은 새 에이전트를 자동 동기화
  - 에이전트/스킬 파일 저장 시: 즉시 개인 폴더로 복사
- CLAUDE.md에 에이전트 추가 시 규칙 명시

### 변경된 파일
- `.claude/hooks/sync-agents.sh` (새 파일), `.claude/settings.json`, `CLAUDE.md`
